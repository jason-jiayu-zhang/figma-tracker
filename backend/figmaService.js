...
async function getFileVersions(fileKey) {
  ...
  while (true) {
    const { versions: batch, nextCursor } = await getFileVersionsPage(fileKey, beforeCursor);
    versions.push(...batch);

    console.log(`[figma] Page ${page}: fetched ${batch.length} versions (total: ${versions.length})`);
