...
async function runSync() {
  ...
  try {
    ...
    try {
      ...
      console.warn(`[sync] Could not fetch /me — continuing without user upsert: ${err.message}`);
      me = null;
    }
    ...
    console.log(`[sync] Tracking ${fileKeys.length} file(s): ${fileKeys.join(', ')}`);
    ...
    for (const fileKey of fileKeys) {
      try {
        ...
        if (fileErr) {
          console.error(`[sync] Failed to upsert file ${fileKey}:`, fileErr.message);
          continue;
        }
        ...
        console.log(`[sync] Found ${versions.length} total versions for ${fileKey}`);
        ...
        console.log(`[sync] ${newVersions.length} new version(s) to insert for ${fileKey}`);
        ...
        if (newVersions.length > 0) {
          ...
          for (const [date, count] of Object.entries(byDate)) {
            ...
            if (existing) {
              ...
              await supabase
                .from('daily_activity')
                .insert({ file_id: fileId, activity_date: date, version_count: count });
            }
          }
        }
        ...
        console.error(`[sync] Error processing file ${fileKey}:`, fileErr.message);
      }
    }
  } catch (err) {
  ...
  // ── 3. Log sync session ──────────────────────────────────────────────────
  const { error: sessionErr } = await supabase.from('sync_sessions').insert(sessionData);
  if (sessionErr) {
...
async function runPageSync() {
  ...
  for (const fileKey of fileKeys) {
    try {
      ...
      if (!currentCursor) {
        const { data: oldest } = await supabase.from('file_versions').select('version_id').eq('file_id', fileId).order('created_at', { ascending: true }).limit(1).maybeSingle();
        currentCursor = oldest ? oldest.version_id : null;
      }

      console.log(`[page-sync] ${fileKey} — fetching 30 versions before: ${typeof currentCursor === 'string' ? currentCursor : JSON.stringify(currentCursor) || '(start)'}`);

      const { versions, nextCursor } = await figma.getFileVersionsPage(fileKey, currentCursor);
      ...
      const { data: existingRows } = await supabase.from('file_versions').select('version_id').eq('file_id', fileId);
      const existingIds = new Set((existingRows || []).map((r) => r.version_id));
      const newVersions = versions.filter((v) => !existingIds.has(v.id));

      console.log(`[page-sync] ${fileKey} — ${newVersions.length} new of ${versions.length} fetched`);
      ...
      if (newVersions.length > 0) {
        ...
        const { error: vErr } = await supabase.from('file_versions').upsert(rows, { onConflict: 'file_id,version_id' });
        if (vErr) {
          console.error(`[page-sync] ${fileKey} — insert error:`, vErr.message);
        } else {
          // Update daily_activity
          const byDate = {};
          for (const v of newVersions) byDate[v.created_at.slice(0, 10)] = (byDate[v.created_at.slice(0, 10)] || 0) + 1;
          for (const [date, count] of Object.entries(byDate)) {
            const { data: existing } = await supabase.from('daily_activity').select('id, version_count').eq('file_id', fileId).eq('activity_date', date).maybeSingle();
            if (existing) {
          ...
          console.log(`[page-sync] ${fileKey} — inserted ${newVersions.length} versions`);
        }
      }

      // Store the next cursor even if 0 new versions found, to move forward
      state[fileKey] = { ...fileState, lastCursor: nextCursor, completed: !nextCursor };
      fs.writeFileSync(statePath, JSON.stringify(state, null, 2));
