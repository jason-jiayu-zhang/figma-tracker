...
export default function Settings() {
  ...
  const saveProfile = async () => {
    ...
    try {
      ...
      await refresh();
      posthog.capture("profile_saved", { public_enabled: publicEnabled });
      setSaved(true);
      setTimeout(() => setSaved(false), 1800);
    } catch (err: any) {
      setSaveError(
        err?.response?.data?.error ||
          "Failed to save. The slug may already be taken.",
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="flex flex-col gap-6 w-full min-h-[768px]">
      ...
      <Card className="flex flex-col gap-5 p-6">
        <h2 className="font-bold text-[18px] tracking-[-0.18px] text-ink">
          Account
        </h2>
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            <h3 className="font-bold text-[18px] tracking-[-0.18px] leading-tight text-black">
              {displayName}
            </h3>
        ...
        <div className="flex flex-col gap-4 w-full max-w-[560px]">
          ...
          <div className="flex flex-col gap-1.5">
            <label
              ...
              className="text-[13px] font-bold text-muted uppercase tracking-wider"
            >
              Profile URL
            </label>
            <div className="flex items-center gap-1 bg-canvas border border-line rounded-xl px-3 py-2 focus-within:border-accent transition-colors">
              <span className="text-[13px] text-muted font-mono whitespace-nowrap">
                {APP_ORIGIN}/u/
              </span>
              <input
                id="profile-slug"
                ...
                aria-describedby={
                  saveError ? "slug-hint slug-error" : "slug-hint"
                }
            ...
            >
              Lowercase letters, numbers, and dashes only. This becomes your
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex flex-col">
              ...
              <span className="text-[12px] text-muted">
                When off, the public URL and embeds return nothing.
              </span>
            </div>
            <button
              type="button"
            ...
            >
              <span
                className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-surface shadow transition-transform ${publicEnabled ? "translate-x-5" : ""}`}
              />
            </button>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            {saved && (
              <span
                role="status"
                className="flex items-center gap-1.5 text-[13px] font-semibold text-green"
              >
                <Check size={15} /> Saved
              </span>
            )}
      ...
      <Card className="flex flex-col gap-5 p-6 border border-red/30">
        ...
        <p className="text-[13px] text-muted leading-relaxed max-w-[560px]">
          This deletes your stored Figma tokens, profile, and every file’s
          tracked history. It cannot be undone. You can reconnect later by
          signing in again.
        </p>
        ...
        {deleteError && (
          <p role="alert" className="text-[12px] text-red font-medium">
            {deleteError}
          </p>
        )}
      </Card>
